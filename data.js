...
// Активная кнопка, у которой сейчас показан номер
let activeOrderBtn = null;

window.showRandomPhone = function(btn) {
    // Если кликнули на другую кнопку, которая уже открыта — ничего не делаем
    if (activeOrderBtn === btn) return;

    // Если до этого была открыта какая-то другая кнопка — возвращаем ей текст «Заказать»
    if (activeOrderBtn && activeOrderBtn !== btn) {
        activeOrderBtn.innerText = "Заказать";
    }
    ...
    // Меняем текст текущей кнопки на номер
    btn.innerText = randomPhone;
    activeOrderBtn = btn; // Запоминаем её как активную
};

// Клик в любое место страницы (кроме активной кнопки) возвращает её в исходное состояние
document.addEventListener('click', function(e) {
    if (activeOrderBtn && !activeOrderBtn.contains(e.target)) {
        activeOrderBtn.innerText = "Заказать";
        activeOrderBtn = null;
    }
});
